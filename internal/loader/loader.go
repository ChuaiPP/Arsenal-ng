...
// externalCheatDir is the user's personal cheat directory.
const externalCheatDir = ".arsenal-ng/cheats"

// LoadExternal reads YAML cheat files from the user's personal cheat directory.
// These cheats are merged with embedded cheats.
func LoadExternal() ([]*model.Cheat, error) {
	home, err := os.UserHomeDir()
	if err != nil {
		return nil, nil // Non-fatal: just use embedded cheats
	}
	dir := filepath.Join(home, externalCheatDir)
	entries, err := os.ReadDir(dir)
	if err != nil {
		return nil, nil // Directory doesn't exist or can't be read
	}

	var extCheats []*model.Cheat
	for _, entry := range entries {
		...
		for _, action := range file.Actions {
			...
			extCheats = append(extCheats, &model.Cheat{
				Tool:     file.Tool,
				Tags:     file.Tags,
				Stage:    stage,
				Tactic:   file.Tactic,
...
func parseCheatFile(path string) ([]*model.Cheat, error) {
	...
	// Determine effective stage: per-action overrides file-level
	stage := file.Stage
