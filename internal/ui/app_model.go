...
type App struct {
	...
	// Persistence manager for history and favorites
	...
	// Favorites list (loaded from disk)
	favorites []state.FavoriteEntry

	// History list (loaded from disk)
	history []model.ExecutionRecord

	// Current mode: normal search (default) vs favorites mode vs history mode
	showFavorites bool
	showHistory   bool
}
