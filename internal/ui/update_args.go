...
func (m App) updateArgs(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.KeyMsg:
		...
		case "ctrl+ ":
			// Cycle through pre-defined options for current argument
			if len(m.selectedCheat.Options) > 0 && m.argCursor < len(m.args) {
				argName := m.args[m.argCursor].Name
				var matches []string
				for _, opt := range m.selectedCheat.Options {
					if opt.Arg == argName {
						matches = append(matches, opt.Value)
					}
				}
				if len(matches) > 0 {
					currentVal := m.argInputs[m.argCursor].Value()
					nextIdx := 0
					...
					m.argInputs[m.argCursor].SetValue(matches[nextIdx])
				}
			}
			return m, nil
