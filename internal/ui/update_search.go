...
func (m App) updateSearch(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.KeyMsg:
		...
		case keyVimSearch:
			// Vim-style: / focuses search and clears query
		...
		case keyFavorites:
			return m.handleFavoritesToggle()
		...
		case keyUp, keyCtrlP, keyVimUp:
			m = m.moveCursorUp()

		case keyDown, keyCtrlN, keyVimDown:
			m = m.moveCursorDown()
...
// handleFilterToggle toggles filter mode.
func (m App) handleFilterToggle() (tea.Model, tea.Cmd) {
	m.filterActive = !m.filterActive
	if m.filterActive {
		m.statusMsg = "Filter mode activated - matching by tool/tag/stage/title/command"
	} else {
		m.statusMsg = "Filter mode deactivated"
	}
	m = m.updateFilter()
	return m, nil
}

// handleHistoryToggle switches between history view and search view.
func (m App) handleHistoryToggle() (tea.Model, tea.Cmd) {
	if m.showHistory {
		m.showHistory = false
		m.filtered = loader.Search(m.cheats, m.searchInput.Value())
		m.cursor = -1
		m.statusMsg = "Showing all cheats"
	} else {
		m.showHistory = true
		m.showFavorites = false
		var histCheats []*model.Cheat
		for _, rec := range m.history {
			histCheats = append(histCheats, &model.Cheat{
				Tool:    rec.Tool,
				Title:   rec.Title,
				Command: rec.Command,
				Tags:    []string{"history"},
				Desc:    fmt.Sprintf("Executed at %d", rec.Timestamp),
			})
		}
		m.filtered = histCheats
		m.cursor = -1
		m.statusMsg = fmt.Sprintf("Showing %d history entries", len(m.history))
	}
	return m, nil
}

// handleFavoritesToggle shows only favorited cheats.
func (m App) handleFavoritesToggle() (tea.Model, tea.Cmd) {
	if m.showFavorites {
		m.showFavorites = false
		m.filtered = loader.Search(m.cheats, m.searchInput.Value())
		m.cursor = -1
		...
		m.filtered = favCheats
		m.cursor = -1
		m.statusMsg = fmt.Sprintf("Showing %d favorites", len(favCheats))
	}
	return m, nil
}

// handleToggleFavorite toggles the current item as favorite.
func (m App) handleToggleFavorite() (tea.Model, tea.Cmd) {
	if len(m.filtered) == 0 || m.cursor < 0 {
		return m, nil
	}
	cheat := m.filtered[m.cursor]
	if m.persistence == nil {
		m.statusMsg = "Persistence not available"
		return m, nil
	}
	if cheat.IsFavorite {
		cheat.IsFavorite = false
		_ = m.persistence.RemoveFavorite(cheat)
		m.statusMsg = fmt.Sprintf("Removed '%s' from favorites", cheat.Title)
	} else {
		cheat.IsFavorite = true
		_ = m.persistence.AddFavorite(cheat)
		m.statusMsg = fmt.Sprintf("Added '%s' to favorites", cheat.Title)
	}
	return m, nil
}
